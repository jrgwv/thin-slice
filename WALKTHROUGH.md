...
**How to prototype an AI app with clean architecture from day one.**
...
See [ARCHITECTURE.md](ARCHITECTURE.md) for full Mermaid diagrams (component, sequence, and layer views).

```
Browser UI
   ↓
FastAPI BFF (validate, delegate, shape)
   ↓
Agent Service (orchestrate prompt → model → parse)
   ↓
Model Integration (Bedrock Converse API → Claude Haiku 4.5)
   ↓
Structured result back to BFF
   ↓
UI render
```
...
3. BFF validates input, generates request ID
4. Agent service builds prompt → calls model → parses response
5. Observer events fire at each stage (for future logging/tracing)
6. Response parser extracts JSON from model output (fences, preamble, or clean)
7. BFF returns structured JSON
8. Frontend renders summary, action items, next step
